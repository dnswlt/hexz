...
func (s *InMemoryPlayerStore) Lookup(playerId PlayerId) (Player, error) {
	s.mut.Lock()
	defer s.mut.Unlock()
...
func (s *InMemoryPlayerStore) Login(playerId PlayerId, name string) error {
	s.mut.Lock()
	defer s.mut.Unlock()
