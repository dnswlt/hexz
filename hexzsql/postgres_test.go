...
func TestPostgresUndoRedo(t *testing.T) {
	if *testPostgresURL == "" {
		t.Skip("Set --test-postgres-url to sth like \"postgres://hexz_test:hexz_test@localhost:5432/hexz_test\" to run this test.")
	}
	ctx := context.Background()
	...
	defer db.Close()
	// Clean up from previous runs.
	if _, err := db.pool.ExecContext(ctx, "DELETE FROM game_history WHERE game_id = 'TestPostgresUndoRedo'"); err != nil {
		t.Fatal("Failed to clean up database: ", err)
	}
	gs := &pb.GameState{
		GameInfo: &pb.GameInfo{
			Id:   "TestPostgresUndoRedo",
			Type: "TestType",
			Host: "Player1",
		},
		Players: []*pb.Player{
	...
	// We shouldn't be able to redo anything so far.
	_, err = db.NextGameState(ctx, gs.GameInfo.Id)
	if err == nil {
		t.Fatal("Unexpected next game state before first move")
	}
	// First move.
	gs.Seqnum++
	seqnumAfterMove := gs.Seqnum
	if err = db.InsertHistory(ctx, "move", gs.GameInfo.Id, gs); err != nil {
		t.Fatal("Failed to insert first move: ", err)
	}
	// Undo.
	...
	// Should be able to redo once.
	next, err := db.NextGameState(ctx, gs.GameInfo.Id)
	if err != nil {
		t.Fatal("Couldn't redo first move: ", err)
	}
	if next.Seqnum != seqnumAfterMove {
		t.Fatalf("Expected seqnum %d, got %d", seqnumAfterMove, next.Seqnum)
	}
	db.InsertHistory(ctx, "redo", gs.GameInfo.Id, nil)
	// ... but not twice.
	_, err = db.NextGameState(ctx, gs.GameInfo.Id)
	if err == nil {
		t.Fatal("Unexpected next game state after single undo")
	}
}
