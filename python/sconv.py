...
def convert(e: hexz_pb2.MCTSExample, validate=True, dtype=np.float32) -> Example:
    ...
    for f in e.board.flat_fields:
        ...
        if f.type == hexz_pb2.Field.NORMAL:
            ...
            raise ValueError("Unknown cell type: " + hexz_pb2.Field.CellType.Name(f.type))
        c += 1
        if c == 10 - r%2:
            r += 1
            c = 0
...
def parse_args():
    parser = argparse.ArgumentParser(description="Hexz Go example to NeuralMCTS example converter.")
    parser.add_argument("--output-format", type=str, default="hdf5",
                        help="Format of the generated output", choices=("hdf5", "html"))
    ...
    for filename in args.files:
        with zipfile.ZipFile(filename, "r") as zf:
            ...
            svg.export(outpath, 
                       ...
                       move_probs=[e.move_probs for e in examples] if args.draw_probs else None)
